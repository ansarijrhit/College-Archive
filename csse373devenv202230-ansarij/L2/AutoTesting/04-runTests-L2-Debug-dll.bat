echo off
rem ******************
rem Runs runs L2.dll test scripts
rem ******************
...
rem ******************
rem First check to see L2.dll exists in the L2\x64\Debug folder 
rem   if not echo a message and exit
rem   if yes then proceed to run test scripts from L2\x64\Debug\L2.dll
rem ******************
IF EXIST "..\x64\Debug\L2.dll" (
"C:\Program Files\Microsoft Visual Studio\2022\Community\Common7\IDE\CommonExtensions\Microsoft\TestWindow\vstest.console.exe" ..\x64\Debug\L2.dll > yourOutput-Debug.txt
) else (
  echo Go back to VS2022 and build L2 in Debug configuration
  echo This will cause VS2022 to create L2.dll in the L2\x64\Debug folder
