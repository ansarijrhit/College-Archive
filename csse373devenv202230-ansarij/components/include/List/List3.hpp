...
void List3<T>::transferFrom(List3& source)
{
	...
	for (int k = 0, z = source.repQ.length(); k < z; k++) {
		T y;
		source.repQ.dequeue(y);
		repQ.enqueue(y);
	}
	repRZ = source.repRZ;
	source.repRZ = 0;
...
void List3<T>::moveToStart(void)
{
	...
	QueueOfT temp, tempL;
	Integer tempLen = repQ.length();

	for (int k = 0, z = repRZ; k < z; k++) {
		T y;
		repQ.dequeue(y);
		temp.enqueue(y);
	}
	for (int k = repRZ, z = tempLen; k < z; k++) {
		T y;
		repQ.dequeue(y);
		tempL.enqueue(y);
	}
...
void List3<T>::swapRights(List3& otherS)
{
	...
	QueueOfT tempQR, tempQL, tempQROther, tempQLOther;
	Integer tempRZ = repRZ;
	Integer tempRZOther = otherS.repRZ;
	...
	tempQL.transferFrom(repQ);
	for (int k = 0, z = tempRZOther; k < z; k++) {
		T x;
		otherS.repQ.dequeue(x);
		tempQROther.enqueue(x);
		otherS.repRZ--;
	}
	tempQLOther.transferFrom(otherS.repQ);

	repRZ = tempQROther.length();
	repQ.transferFrom(tempQROther);

	for (int k = 0, z = tempQL.length(); k < z; k++) {
		T x;
		tempQL.dequeue(x);
		repQ.enqueue(x);
	}

	otherS.repRZ = tempQR.length();
	otherS.repQ.transferFrom(tempQR);

	for (int k = 0, z = tempQLOther.length(); k < z; k++) {
		T x;
		tempQLOther.dequeue(x);
		otherS.repQ.enqueue(x);
	}
